 ...
 * 
 */
public class StockTradeReaderImplFailureTest {
	private static Logger log = LogManager.getLogger(StockTradeReaderImplFailureTest.class);
	...
	public void setUp() throws StockTradeInvalidFormateException {
		log.info("This is inside the SetUp of failure testcase class method");
	...
	public void testGetMaxVolumeTrade() {
		StockTrade MaxVol = reader.getMaxVolumeTrade(list);
		String s  ="" + MaxVol.getVol();
		Assert.assertEquals(s, "2.4325548");
	...
	public void testGetMinVolumeTrade() {
		StockTrade MinVol = reader.getMinVolumeTrade(list);
		String s  ="" + MinVol.getVol();
		Assert.assertEquals(s, "1.2507E7");
	...
	public void testGetDailyTradingDifferential() {
		...
		String expected = "" + ((list.get(0).getHigh()-list.get(1).getOpen()));
